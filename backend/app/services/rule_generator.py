...
def generate_rules_with_comprehensive_engine(
    ...
    def _run_one(column_name: str) -> Dict[str, Any]:
        ...
        prompt = generate_comprehensive_ai_prompt(
            ...
            sibling_samples=sibling_samples_by_col[str(column_name)],
        )

        try:
            # temperature=0 + fixed seed + json_object response_format pin
            # the model to deterministic, parseable output across reruns.
    ...
    results_by_col: Dict[str, Dict[str, Any]] = {}
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        futures = {pool.submit(_run_one, c): c for c in df.columns}
        for fut in as_completed(futures):
            res = fut.result()
            results_by_col[res["column"]] = res

    # Re-emit rules in df.columns order so S.No is deterministic and not
    # dependent on which thread happened to finish first.
    all_rules: List[Dict[str, Any]] = []
    for column_name in df.columns:
        res = results_by_col.get(column_name)
        if not res:
            continue
